...
TTerm 自动读取 `~/.ssh/config`，让已有的远程开发环境直接成为终端 Profile。内置 SSH 客户端支持密码、密钥、主机指纹确认以及本地、远程和 SOCKS5 端口转发；会话发生短暂传输中断时可以自动恢复并保留历史。
...
- **SSH**：自动读取 `~/.ssh/config`，支持密码、密钥、主机指纹确认以及本地、远程和 SOCKS5 端口转发。
