...
- **Design tokens**: `--tt-*` in `src/ui/tokens.css` (skins `body[data-skin]`, `body.qp-glass`). `--term-bg` is JS-written per terminal scheme (2px seam) — NOT a `--tt-*` token. Tab chrome follows chrome skin (same surface as Settings sidebar), never terminal schemes. Settings is strictly 1:1 with `docs/settings-preview.html` (148px `--tt-btn-width`, row wells `.row`, visibility = LEFT checkbox `.check-row`, toggles only for single on/off).
