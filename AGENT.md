...
- Window state save/restore is handled entirely by `tauri-plugin-window-state`. Do NOT write custom save/restore code. After restore, setup defers once: **first launch** (no usable `.window-state.json`) sizes the window to the monitor × `(1/φ)` ≈ 61.8% and centers it; then `window::enforce_min_size` re-applies the 800×600 floor (plugin `set_size` bypasses the OS min-track). Do **not** clamp on every `Resized` — that aborts maximize on undecorated Windows.
...
- Agent TUIs (pi, Cursor agent, Claude Code) hide the hardware cursor (`ESC[?25l`) and draw a fake one. xterm's IME support assumes the real cursor is the input point → pinyin composition renders in a corner or nowhere. Win10 inbox ConPTY often swallows DECTCEM, so the unique-short-run scan also runs while the hardware cursor looks visible — **only on Win10**, and only when the probe says hide was not forwarded. Caps live in `conpty-ime.json` (not `config.json`), stamped with `appVersion` so an update re-probes and heals. Settings → IME fake-cursor scan (default on) is the user off-ramp.
...
Frontend → backend commands (invoke): the single source of truth is the `tterm_commands!` macro in `src-tauri/src/lib.rs` (one list for both build profiles; debug builds append `demo_spawn`/`anime_spawn`). Notable families: `pty_*` (spawn/resize/kill), `pty_probe_ime_caps` (Win10 ConPTY DECTCEM probe), `window_*` (controls + maximize/fullscreen for zen), `read/write/delete_config_file` (per-topic JSON: config / themes / serial-profiles / keybindings / ssh-history / conpty-ime), `ssh_*` + `sshclient::ssh_*` (config file, embedded client, forwards, keygen), `serial_*`, `share_*`, `tray_*`, and `open_new_window` / `pick_directory` / `save_text_file`.
