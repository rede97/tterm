...
- Menu layout: one submenu per parked window — `N#Tab M` (window N in park order, M = tab count; windows have no meaningful title) — listing that window's tab labels; clicking a tab restores the window AND activates that tab. Plus `Quit TTerm` (terminates every parked process; shells die with it).
