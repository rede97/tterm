...
- **Windows Terminal 配置导入** — 读取 WT 的 `settings.json`，复用已有配置，包括 VS Developer Prompt / PowerShell
- **管理员模式** — 按住 Shift 点击菜单项即可提权启动，或右键标签复制为管理员
- **标签上下文菜单** — 右键标签可改名、换颜色、复制、导出文本、关闭右侧/其他
- **终端内搜索** — Ctrl+Shift+F 打开搜索栏
- **自定义窗口装饰** — 无原生标题栏，VS Code 风格标签栏集成窗口控制按钮
- **配置持久化** — 默认终端类型等设置跨会话保留
