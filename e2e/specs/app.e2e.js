describe("TTerm application", () => {
  ...
  it("demo TTY tab renders TUI frames and OSC 9;4 progress bar", async () => {
    ...
    const demoItem = await browser.waitUntil(
      async () => {
  ...
  it("abnormal shell exit shows the in-band reconnect prompt; clean exit auto-closes the tab", async () => {
    ...
    await browser.execute(async () => {
      await window.__tterm.mgr.createLocalTab();
    });
    ...
    await browser.waitUntil(
      async () => {
        return await browser.execute(() =>
          [...document.querySelectorAll("#tabs .tab")].some((t) =>
            t.classList.contains("disconnected"),
          ),
        );
      },
    ...
    await browser.waitUntil(
      async () => {
        return await browser.execute(() =>
          [...document.querySelectorAll("#tabs .tab")].every(
            (t) => !t.classList.contains("disconnected"),
          ),
        );
      },
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 5000,
        timeoutMsg: "disconnect prompt was lost on respawn (not scrolled into scrollback)",
      },
    );
  ...
  it("reorders tabs via drag (SortableJS)", async () => {
    ...
    const startX = Math.round(t2.x),
      endX = Math.round(t1.x - 60),
      y = Math.round(t1.y);
    const action = browser.action("pointer").move({ x: startX, y }).down().pause(120);
    const steps = 8;
    for (let i = 1; i <= steps; i++) {
      action
        .move({ x: Math.round(startX + ((endX - startX) * i) / steps), y, duration: 60 })
        .pause(40);
    }
  ...
  it("mock serial loopback echoes typed input (debug enumeration)", async () => {
    ...
    await browser.waitUntil(
      async () => {
        return await browser.execute(() => {
          ...
          return items.some((i) =>
            i.querySelector(".item-label")?.textContent?.includes("MOCK-LOOP"),
          );
        });
      },
      { timeout: 8000 },
    ...
    await browser.execute(() => {
      for (const [, tab] of window.__tterm.tabs.entries()) {
        if (tab.label?.includes("MOCK")) tab.terminal.focus();
      }
    });
    ...
    await browser.waitUntil(
      async () => {
        return await browser.execute(() => {
          const items = [...document.querySelectorAll(".profile-menu .profile-item")];
          return items.some((i) =>
            i.querySelector(".item-label")?.textContent?.includes("MOCK-NL"),
          );
        });
      },
      { timeout: 8000 },
  ...
  it("shift+right-click Clear empties scrollback but keeps the prompt line", async () => {
    ...
    await browser.waitUntil(async () => (await dumpBuffer()).includes("CLEARMARK2"), {
      timeout: 10000,
    });
    ...
    await browser.waitUntil(
      async () => {
        return await browser.execute(() => {
          ...
          return (
            !!menu &&
            [...menu.querySelectorAll(".menu-item")].some((i) => i.textContent === "Clear")
          );
        });
      },
    ...
    await browser.waitUntil(async () => (await dumpBuffer()).includes("AFTERCLEAR"), {
      timeout: 10000,
      timeoutMsg: "shell did not echo after clear (prompt lost?)",
    });
  });
  ...
  it("tabs are equal width and carry the full name in the hover tooltip", async () => {
    ...
    // any moment, CAPPED at 180px even when a single tab could fill the
    ...
    expect(single).toBeLessThanOrEqual(180);
    expect(single).toBeGreaterThanOrEqual(108);
    ...
    for (const t of info) {
      expect(t.w).toBeLessThanOrEqual(180);
  ...
  it("keeps + and dropdown pinned when tabs overflow into the scrollbar", async () => {
    ...
    await browser.waitUntil(
      () =>
        browser.execute(() => {
          const tabs = document.getElementById("tabs");
          return tabs.classList.contains("overflowing") && tabs.scrollWidth > tabs.clientWidth + 1;
        }),
    ...
    await browser.execute(() => {
      const mgr = window.__tterm.mgr;
      const demoIds = [...mgr.tabs.values()].filter((t) => t.label === "Demo TTY").map((t) => t.id);
