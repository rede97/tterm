...
// Compiled out entirely in release builds (zero binary cost).
#[cfg(debug_assertions)]
mod demo {
    use super::*;

    pub(super) struct DemoReader {
    ...
    pub(super) struct DemoWriter {
        pub(super) tx: std::sync::mpsc::Sender<u8>,
    }
...
//   380..400 hidden (state 0), done message
    pub(super) fn render_demo_frame(tick: u64) -> Vec<u8> {
...
#[tauri::command]
#[cfg(debug_assertions)]
fn demo_spawn(state: tauri::State<AppState>) -> Result<WsConnectResult, String> {
    use demo::*;
