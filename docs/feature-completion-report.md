...
| SSH / 串口自动重连 | ✅ 达成（回车手动重连） | `SpawnSpec` 记录会话参数；子进程看门狗（`child.wait()` → 置 `master=None` 解除 ConPTY 读阻塞，nonce 防重连竞态）；relay 在流结束时主动发送 WS Close 帧；Tab 断联遮罩 + 标签划线标红，回车调 `session_reconnect` 同 id 重建。E2E 端到端验证（exit → 遮罩 → 回车 → 恢复） |
