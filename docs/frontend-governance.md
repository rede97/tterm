...
> **第四轮审计（基线 `7d8c8e5`，v2.0.0 后两个提交）**：sshclient 拆分验证为"纯移动 + 声明修复"无夹带；L8–L13 修复 6/6 回源码确认；循环 import 清零、硬规则机器化；前端规整度 8.5→8.8/10，**后端审计债清零**（四轮累计 1 高 + 9 中 + 13 低，除配置原子写外全部修复）。新挂账见末节"第四轮剩余项"。
